...
# added
# had to run pip install psycopg2-binary
...
# **************************************************************************************
#   Set to get last 365 days - only getting 100 records at most
# **************************************************************************************
...
# tried the following to get 500 records. Docs say you can keep calling and offsetting by amount you load
    # transactions_response = client.Transactions.get(access_token, start_date, end_date, count=count, offset=offset)
    # transactions_response = client.Transactions.get(access_token, start_date, end_date, {
        # count: count,
        # offset: offset
        # count: 500,
        # offset: 0
    # })
    # below error integer does not support item assignment?
    # transactions_response = client.Transactions.get(access_token, start_date, end_date, count, offset)



    # just want the transaction dictionary
    transactions = transactions_response['transactions']

    # create database session to connect to postgresql
    # postgres username = postgres
    # sample: dialect+driver://username:password@host:port/database
    db_string = 'postgresql://postgres:BestJob15!@localhost:5432/Plaid'

    # instantiate the database and create an engine
    ...
    # count transactions added to the database
    total = 0
    # loop through transaction response and add to db
    for tran in transactions:
        ...
        total += 1
        print(str(tran['transaction_id']) + "Transaction added to the database: " + str(total))
